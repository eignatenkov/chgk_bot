...
from rating_tools import (
    find_team_by_name,
    get_tournament_results_by_id,
...
class FindTeamByNameTestCase(unittest.TestCase):
    def test_happy_pass(self):
        ...
        self.assertTrue(
            set(output_team.keys())
            == {"idteam", "name", "town", "region_name", "country_name"}
        )
...
class GetTournamentResultsByIdTestCase(unittest.TestCase):
    def test_happy_pass(self):
        ...
        first_team = output[0]
        self.assertEqual(
            first_team["idteam"], "407", msg="results are not sorted by team id!"
        )
        expected_first_team = {
            "idteam": "407",
